    ...
    <div class="body-wrapper">
        <div class="bodywrapper__inner">
            <div class="d-flex mb-30 flex-wrap gap-3 justify-content-between align-items-center">
                <h6 class="page-title">Edit purchase</h6>
                <div class="d-flex flex-wrap justify-content-end gap-2 align-items-center breadcrumb-plugins">
                    {{-- <a href="{{ route('all-purchases') }}" class="btn btn-sm btn-outline--primary">
                                ...
                                <div class="row mb-3">
                                        ...
                                        <div class="form-group" id="supplier-wrapper">
                                            ...
                                            <select name="supplier" class="form-control" required>
                                                @foreach($Suppliers as $supplier)
                                                    <option value="{{ $supplier->name }}" {{ $supplier->id == $purchase->supplier ? 'selected' : '' }}>
                                                        {{ $supplier->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-xl-4 col-sm-6">
                                        <div class="form-group">
                                            <label>Date</label>
                                            <input name="purchase_date" type="date" class="form-control bg--white" value="{{ $purchase->purchase_date }}" required>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="warehouse_id" class="form-control" required>
                                                @foreach($Warehouses as $Warehouse)
                                                    <option value="{{ $Warehouse->name }}" {{ $Warehouse->name == $purchase->warehouse_id ? 'selected' : '' }}>
                                                        {{ $Warehouse->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="row mb-3">
                                    <div class="table-responsive">
                                        <table class="productTable table border">
                                            ...
                                            {{-- @php
                                            $itemCategories = json_decode($purchase->item_category);
                                            $quantities = json_decode($purchase->quantity);
                                            $prices = json_decode($purchase->price);
                                            $totals = json_decode($purchase->total);
                                        @endphp --}}
                                        ...
                                        <tbody id="purchaseItems">
                                            ...
                                            <tr>
                                                <td>
                                                    <select name="item_category[]" class="form-control item-category" required>
                                                        @foreach($Category as $Categories)
                                                            <option value="{{ $Categories->id }}" {{ $Categories->id == $purchase->item_category[$index] ? 'selected' : '' }}>
                                                                {{ $Categories->category }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <td>
                                                    <input type="text" name="unit[]" class="form-control" value="{{ $purchase->unit[$index] ?? '' }}">
                                                </td>

                                                <td><input type="number" name="quantity[]" class="form-control quantity" value="{{ $purchase->quantity[$index] }}"></td>
                                                <td><input type="number" name="price[]" class="form-control price" value="{{ $purchase->price[$index] }}"></td>
                                                <td><input type="number" name="total[]" class="form-control total" value="{{ $purchase->total[$index] }}" readonly></td>
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12">
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <input type="number" name="total_price" class="form-control total_price" value="{{ $purchase->total_price }}" readonly>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-sm-12">
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <span class="input-group-text">Pkr</span>
                                                        <input type="number" name="discount" id="discount" class="form-control" value="{{ $purchase->discount }}">
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-sm-12">
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <span class="input-group-text">Pkr</span>
                                                        <input type="number" name="payable_amount" class="form-control payable_amount" value="{{ $purchase->Payable_amount }}" readonly>
                                                    </div>
                                                </div>
                                            </div>
